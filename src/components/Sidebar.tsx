...
export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabChange }) => {
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-0 h-screen w-[280px] flex-col bg-[#FFF9F2] border-r border-[#7A2332]/15 z-50 shadow-sm">
      {/* Header / Logo */}
      <div className="p-6 flex flex-col items-center text-center gap-2 border-b border-[#7A2332]/10 bg-gradient-to-b from-[#7A2332]/5 to-transparent">
      ...
      {/* User Profile Footer */}
      <div className="p-4 border-t border-[#7A2332]/10 flex items-center gap-3 bg-[#F8ECE0]/60">
        <img 
          alt="Profile" 
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-label-caps text-xs text-[#2D2118] font-bold truncate">Ana Maria</span>
          <span className="text-[11px] text-[#5C4A3E] truncate">Coordenação de Música</span>
        </div>
      </div>
    </aside>
  );
